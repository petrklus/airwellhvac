...
void sendACCommand() {
  ...
  for (int i=0; i<sizeof(autoFan22Heat)/2; i++) {    
    pulseIR(autoFan22Heat[i*2]*10);
    delayMicroseconds(autoFan22Heat[i*2+1]*10);
  }
...
void loop(void) {
  ...
  if (IRcompare(numberpulses, ACOnOff,sizeof(ACOnOff)/4)) {
    Serial.println("ACONOFF");
